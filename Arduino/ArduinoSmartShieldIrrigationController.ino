...
int noOfStations = 5; //number of noOfStations or relays
...
long updateSTHubEveryXMin = 10L;
long checkQueueEveryXMin = 10L;

boolean isDebugEnabled = true;
boolean isSTLEDEnabled = true;

int timeoutLED = 200;	//ms

void setup()
{
	Serial.begin( 9600 );         // setup serial with a baud rate of 9600
	writeToSerialPortDebug( "Inside setup() ..." );

	indicateUsingSTLED( 1, 0, 0 );

	// setup timed actions
	timer.every( updateSTHubEveryXMin * 60L * 1000L, timeToUpdate ); //send update to smartThings hub every 10 min
	timer.every( checkQueueEveryXMin * 60L * 1000L, queueManager ); // check queue to see if there is work to do
...
{
	writeToSerialPortDebug( "Received message: '" );
	writeToSerialPortDebug( message );
	writeToSerialPortDebug( "' \n" );

	indicateUsingSTLED( 0, 0, 1 );

	char * inValue[ noOfStations + 2 ]; //array holds any values being delivered with message (1-8) and NULL; [0] is not used
	char delimiters[] = ",";
	...
	{
		inValue [ i ]
		= strtok( NULL, delimiters ); //remove remaining substrings as incoming values
		i++;
	}
	if ( strcmp( inValue[ 0 ], "on" ) == 0 )
	{   // add new station to queue
		if ( strcmp( inValue[ 2 ], "0" ) != 0 && strcmp( inValue[ 2 ], "null" ) != 0 )
		{  //do not add to queue if zero time
	...
	{
		int i = 1;
		while ( i < noOfStations + 1 )
		{
			if ( i != trafficCop )
			{
				if ( strcmp( inValue[ i ], "0" ) != 0 && strcmp( inValue[ i ], "null" ) != 0 )
				{  //do not add to queue if zero time
					queue[ i ] = 1;
				}
			}
	...
	indicateUsingSTLED( 0, 1, 0 );
	writeToSerialPortDebug( "Message processed." );
}
...
void toggleOn()
{
	...
	timer.stop( stationTimer[ trafficCop ] ); // Kill any previously started timers.
	stationTimer[ trafficCop ] = timer.after ( stationTime[ trafficCop ], toggleOff );
	scheduleUpdate();
}
void toggleOff()
{
...
void writeToSerialPortDebug( String strIn )
{
	if ( isDebugEnabled )
	{
		Serial.print( strIn );
...
void indicateUsingSTLED( int r, int g, int b )
{
	if ( isSTLEDEnabled )
	{
		smartthing.shieldSetLED( r, g, b );
		delay( timeoutLED );
